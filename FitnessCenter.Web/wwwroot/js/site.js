...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!allowedRoutes.includes('*') && !allowedRoutes.includes(currentPath)) {
    ...
    } else if (!allowedRoutes.includes('*')) {
        // Deshabilitar los enlaces a vistas no permitidas
        document.querySelectorAll('a').forEach(link => {
            const href = link.getAttribute('href');
            if (href && !allowedRoutes.includes(new URL(href, window.location.origin).pathname)) {
                disableLink(link);
            }
        });
    }
